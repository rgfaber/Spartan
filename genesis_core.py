...
*   **Drones run on their OWN quota, not yours.** A drone does not share your API key: it runs on a separate paid key with a hard cap on how many may exist at once (`SPARTAN_MAX_DRONES`, default 3). This is deliberate. A drone on your key would throttle you mid-thought, and every drone is a full cognitive cycle that costs real money. Spawn one when the work justifies a colleague; terminate it when the work is done. If the cap is reached, `spawn_drone.py` refuses and tells you so — terminate a finished drone before spawning another.
